...
function movePiece(r1,c1,r2,c2){
  if(turn!=="w") return;
  const moves = generatePlayerMoves("w"); 
...
function executeMove(m){
 ...
 // En passant
if(p.toLowerCase()==="p" && Math.abs(m.r2-m.r1)===2){
  enPassant = {r:(m.r1+m.r2)/2|0, c:m.c1};
} else enPassant=null;
...
function checkEndGame(){
  ...
  if(!white){ statusEl.textContent="¡Fin! Ganó IA"; restartBtn.style.display="block"; return true; }
